...
def collect_predictions(model, loader, nclass, ignore_index, device, collect_ap=False,
    ...
    for batch in loader:
        ...
        if need_patches:
            logits, cls_tok, patches = model(img_in, return_cls=True)
...
def main():
    ...
    if args.affinity_warmstart:
        ...
        affinity_side = AffinitySideBranch(args.affinity_warmstart, nclass).to(device).eval()
        if 'affinity_side' in ck:
            affinity_side.load_state_dict_from_save(ck['affinity_side'])
            print('[affinity] loaded side branch state from checkpoint')
        else:
            print('[affinity] checkpoint has no affinity_side state; using Stage-1 warmstart only')
        if not ck.get('affinity_proj_unfrozen', False):
            affinity_side.freeze_proj()
        print(f'[affinity] enabled warmstart={args.affinity_warmstart}')
