...
def _split_line(line):
    # Support both space and tab separators between image and mask paths
...
class SemiDataset(Dataset):
    def __init__(self, name, root, mode, size=None, id_path=None, nsample=None):
        ...
        if mode == 'train_l' or mode == 'train_u':
            ...
            val_path = id_path if id_path else 'splits/%s/val.txt' % name
            with open(val_path, 'r') as f:
                self.ids = f.read().splitlines()

    def __getitem__(self, item):
        id = self.ids[item]
        img_rel, mask_rel = _split_line(id)
        ...
        if self.mode == 'train_l':
            img, mask = crop_with_valid_pixel(img, mask, self.size, ignore_value)
        else:
