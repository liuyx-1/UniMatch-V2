...
# so unzip on the server produces /root/autodl-tmp/code/UniMatch-V2/ rather than
# scattering unimatch_v2.py, configs/, etc. into the code root.
...
function Print-UnpackCommand {
    Write-Host ""
    Write-Host "On the new AutoDL server, unpack with:"
    Write-Host "  mkdir -p /root/autodl-tmp/code && cd /root/autodl-tmp/code && rm -rf ${RepoName}.old && \"
    Write-Host "    [ -d ${RepoName} ] && mv ${RepoName} ${RepoName}.old; \"
    Write-Host "    unzip -q /root/autodl-tmp/code/$(Split-Path $Zip -Leaf) -d /root/autodl-tmp/code"
}

if (-not $Upload) {
    Write-Host "[done] re-run with -Upload to push to bitahub."
    Write-Host "[scp] fallback: scp this zip to /root/autodl-tmp/code/ before unpacking."
    Print-UnpackCommand
    exit 0
}
