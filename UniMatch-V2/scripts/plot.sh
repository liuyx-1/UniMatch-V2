...
#
# Plots the main variants (base/text/boundary/full) at the given label rate.
...
if [ -n "$RUNS" ]; then
    ...
    python tools/plot_train_log.py \
        ...
        --run "v2 text:${ROOT}/unimatch_v2_affinity_min_r${RATE}${SUFFIX}_joint/train_log.csv" \
        --run "v3 boundary:${ROOT}/unimatch_v2_boundary_r${RATE}${SUFFIX}/train_log.csv" \
        --run "v4 full:${ROOT}/unimatch_v2_full_r${RATE}${SUFFIX}_joint_edge_bnd/train_log.csv" \
