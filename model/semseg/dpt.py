...
class DPT(nn.Module):
    ...
    def forward(self, x, comp_drop=False, return_cls=False):
        ...
        if return_cls:
            ...
            cls_tok = out_pairs[-1][1]      # last-layer CLS [B, embed_dim]
        else:
            features = self.backbone.get_intermediate_layers(
                x, self.intermediate_layer_idx[self.encoder_size]
            )

        # Last-layer patch tokens BEFORE comp_drop, exposed for affinity side branch.
